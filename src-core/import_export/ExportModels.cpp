...
// wx-free port of xLightsFrame::ExportModels (src-ui-wx/import_export/ExportEffects.cpp).
// Shared by the desktop UI and the iPad bridge.
...
// wx-free equivalent of xLightsFrame::GetControllerDetailsForChannel — pure
// OutputManager/Controller/Output logic, so it lives in core.
void GetControllerDetailsForChannel(OutputManager& outputManager, int32_t channel, std::string& controllername, std::string& type, std::string& protocol, std::string& description, int32_t& channeloffset, std::string& ip, std::string& u, std::string& inactive, std::string& baud, int& start_universe, int& start_universe_channel) {
    int32_t sc = 0;
    int32_t csc = 0;
    Controller* c = outputManager.GetController(channel, csc);
    Output* o = outputManager.GetOutput(channel, sc);
    ...
    if (c != nullptr) {
        ...
        if (ser != nullptr) {
            ip = ser->GetPort();
            baud = std::to_string(ser->GetSpeed());
        }
...
std::string JoinAliases(const std::list<std::string>& aliases) {
    if (aliases.empty()) {
        return "";
    }
...
bool ExportModels(const std::string& filename, ModelManager& allModels, OutputManager& outputManager) {
    ...
    if (workbook == nullptr) {
        spdlog::error("ExportModels: unable to create workbook at {}", filename);
        return false;
    }
    ...
    auto write_worksheet_string = [FACTOR](lxw_worksheet* sheet, int row, int col, std::string text, lxw_format* fmt, std::map<int, double>& col_widths) {
        worksheet_write_string(sheet, row, col, text.c_str(), fmt);
    ...
    for (auto const& m : allModels) {
        ...
        if (model->GetDisplayAs() != DisplayAsType::ModelGroup) {
            modelCount++;
            std::string const stch = model->GetModelStartChannel();
            ...
            write_worksheet_string(modelsheet, row, 14, fmt::format("#{}:{}", stu, stuc), format, _model_col_widths);
            worksheet_write_number(modelsheet, row, 15, model->GetLastChannel() + 1, format);
            write_worksheet_string(modelsheet, row, 16, fmt::format("{} x {}", w, h), format, _model_col_widths);
            ...
            std::string con_attributes = model->GetControllerConnectionAttributeString();
            Replace(con_attributes, ":", ",");
            if (StartsWith(con_attributes, ",")) {
                con_attributes.erase(0, 1);
            }
    ...
    for (auto const& m : allModels) {
        ...
        if (model->GetDisplayAs() == DisplayAsType::ModelGroup) {
            ...
            write_worksheet_string(groupsheet, row, 3, fmt::format("{} x {}", w, h), format, _group_col_widths);
            write_worksheet_string(groupsheet, row, 4, model->GetLayoutGroup(), format, _group_col_widths);
            std::string aliases = JoinAliases(model->GetAliases());
            if (!aliases.empty()) {
                write_worksheet_string(groupsheet, row, 5, aliases, format, _group_col_widths);
            }
            ++row;
        }
    }
    ...
    for (const auto& it : outputManager.GetControllers()) {
        auto columns = Split(it->GetExport(), ',');
    ...
    if (error) {
        spdlog::error("ExportModels: unable to create spreadsheet, error {} = {}", (int)error, lxw_strerror(error));
        return false;
    }
    return true;
}
